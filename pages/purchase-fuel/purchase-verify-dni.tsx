...
const PurchaseVerifyDni = () => {
  ...
  const goOperationComplete = () => {
    navigation.reset([
      ...
      NavigationActions.navigate({ routeName: getRoutePath(HOME_ROUTE.PurchaseResult, HOME_ROUTE), params: { operationId: operationId } })
    ], 1);
  };

  const goOperationRejected = () => {
    navigation.navigate(
      getRoutePath(HOME_ROUTE.PurchaseDniRejected, HOME_ROUTE),
    );
  };
        ...
        <InfoPanel>
          <CenteredContainer style={{ flex: 1, justifyContent: 'center' }}>
            <View>
              ...
              <Title>Presente su DNI junto con el monto a pagar</Title>
              <Subtitle>
                Para su seguridad, necesitamos verificar su identidad, le
                solicitamos presente su DNI al playero junto con el efectivo. Una vez verificada su
                identidad podrá finalizar su compra.
              </Subtitle>
            </View>
          </CenteredContainer>
        </InfoPanel>
...
PurchaseVerifyDni.navigationOptions = {
  header: null,
};

export default PurchaseVerifyDni;
